...
- 执行与运行记录追踪
- gate 门禁感知的状态推进
- evidence-first 的全流程状态历史
- API/SSE/CLI/控制台统一的 ledger 事件契约
...
Stage Board + 任务详情（同图示意）：

![AgentFlow Stage Board 与任务详情演示](./docs/images/stage-board-task-detail-demo.svg)

## 当前已实现能力

### 核心引擎（CLI + DB）

- SQLite 优先存储：`projects`、`tasks`、`runs`、`triggers`、`gate_profiles`、`ledger_events`
...
- 任务详情（统一事件时间线 `timeline` + 派生信号 `derived_summary`）
- 人工流转（带规则校验，支持 `force`）
...
- `agentflow audit --project <project>`：读取统一项目审计事件
- `agentflow run-steps <run_id>`：命令名保持兼容，底层改为 run timeline ledger 事件
- `agentflow recent-runs --project <project>`：保留 run 快照，同时输出每次 run 的最新 ledger 事件摘要
...
- 可追踪：`ledger_events` 是事实源，`tasks/runs` 保留为当前态快照
- 多仓库友好：一个控制面管理多个仓库项目
