...
class MockAdapter:
    ...
    def execute(self, context: AdapterContext, agent_name: str) -> AdapterResult:
        ...
        if "design" in title or "proposal" in title:
            return AdapterResult(success=True, note=f"{agent_name} prepared design notes", to_status="ready")
        return AdapterResult(success=True, note=f"{agent_name} produced implementation plan", to_status="review")
